...
for dataset in mydata:
    ...
    while i < len(segmentation)-1:
        ...
        for goal in goals:
            features = []
            for key in mykeys:  # mykeys
                euc_distance = objects[goal][current_segment[0]:current_segment[1],
                                             0:3] - positions[key][current_segment[0]:current_segment[1]]
                orientation_distance = orientations[key][current_segment[0]:current_segment[1]]
                ...
                orientation_distance = np.linalg.norm(
                    orientation_distance-euc_distance/np.linalg.norm(euc_distance, axis=1)[:, None], axis=1)
                euc_distance = np.linalg.norm(euc_distance, axis=1)
                # print(key)
                # print(euc_distance)
                # print(orientation_distance)
                features.append([key + " euc", euc_distance])
                features.append([key + " ori", orientation_distance])

                # plt.plot(orientation_distance)
                # plt.plot(euc_distance)
                # plt.show()
            goggles_distance = objects[goal][current_segment[0]:current_segment[1],
                                             0:3] - objects['goggles'][current_segment[0]:current_segment[1], 0:3]
            goggles_distance = goggles_distance / \
                np.linalg.norm(goggles_distance, axis=1)[:, None]
            # print(gaze)
            gaze_distance = gaze[current_segment[0]:current_segment[1]]
            gaze_distance = np.linalg.norm(
                gaze_distance-goggles_distance, axis=1)
            features.append(["gaze", gaze_distance])
            features.append(["label", goal == curr_goal])
            with open(f'../data/processed/{dataset}/{str(num_segments)}/{goal}.csv', mode='w') as f:
                my_writer = csv.writer(
                    f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                for item in features:
                    my_writer.writerow(item)
            num_examples += 1
            # plt.plot(gaze_distance)
            # plt.show()
            # print(gaze_distance)
